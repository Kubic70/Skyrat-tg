		...
		switch(user.zone_selected) //to let code know what part of body we gonna tickle
			if(BODY_ZONE_PRECISE_GROIN)
				if(M.is_bottomless())
					...
					if(penis)
						message = (user == M) ? pick("leans the shocker against their penis, letting it shock it. Brutal.","shocks their penis with [src]") : pick("uses [src] to shock [M]'s penis", "shocks [M]'s penis with [src]","leans the shocker against [M]'s penis, turning shocker on")
						M.emote(pick("twitch","twitch_s","shiver"))
						M.do_jitter_animation()
						M.adjustStaminaLoss(7)
						M.adjustArous(0,0,9)
					...
					if(vagina)
						...
						message = (user == M) ? pick("leans the shocker against their belly, letting it shock it.","shocks their tummy with [src]") : pick("uses [src] to shock [M]'s belly", "shocks [M]'s belly with [src]","leans the shocker against [M]'s tummy, turning shocker on")
						M.emote(pick("twitch","twitch_s","shiver"))
						M.do_jitter_animation()
						M.adjustStaminaLoss(7)
						M.adjustArous(0,0,9)
